...
class SegmentClient {
    constructor(_opts) {
        const key = (_opts === null || _opts === void 0 ? void 0 : _opts.key) || (0, common_all_1.env)("SEGMENT_VSCODE_KEY");
    ...
    async trackInternal(event, properties, context, timestamp) {
        return new Promise((resolve) => {
            ...
            this._segmentInstance.track(payload, (err) => {
                if (err) {
        ...
        eventLines.forEach((line) => {
            const singleTry = new Promise((resolve) => {
                try {
                    ...
                    const promised = this.trackInternal(data.event, data.properties, data.context, data.timestamp);
                    promised
                        .then((resp) => {
                        resolve(resp.error
...
class SegmentUtils {
    static track(event, platformProps, props) {
        if (common_all_1.RuntimeUtils.isRunningInTestOrCI()) {
            return;
        }
    ...
    static async trackSync(event, platformProps, props) {
        if (common_all_1.RuntimeUtils.isRunningInTestOrCI()) {
            return;
        }
        const { type, ...rest } = platformProps;
