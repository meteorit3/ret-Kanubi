...
opts) {
    ...
    const nameClean = name || common_all_1.env("LOG_NAME", { shouldThrow: false }) || "logger";
    const logDst = dest || common_all_1.env("LOG_DST", { shouldThrow: false }) || "stdout";
    const pinoOpts = { name: nameClean, level };
    if (logDst === "stdout") {
        return pino_1.default(pinoOpts);
    }
    else {
        return pino_1.default(pino_1.default.destination(logDst)).child(pinoOpts);
    }
}
