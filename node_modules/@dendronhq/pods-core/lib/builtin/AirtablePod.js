...
class AirtableUtils {
    ...
    static checkNoteHasAirtableId(note) {
        return !lodash_1.default.isUndefined(lodash_1.default.get(note.custom, "airtableId"));
    }
    static filterNotes(notes, srcHierarchy) {
        return notes.filter((note) => note.fname.includes(srcHierarchy));
    }
    static getAirtableIdFromNote(note) {
        return lodash_1.default.get(note.custom, "airtableId");
    }
    /***
    ...
    static handleSrcField({ fieldMapping, note, engine, }) {
        ...
        switch (type) {
            ...
            case "date": {
                return {
                    data: engine_server_1.NoteMetadataUtils.extractDate({ note, key }),
                };
            }
            case "singleSelect": {
            ...
            case "linkedRecord": {
                ...
                const recordIds = links.flatMap((l) => {
                    ...
                    const vault = vaultName
                        ...
                        : undefined;
                    const _notes = common_all_1.NoteUtils.getNotesByFname({ fname, notes, vault });
    ...
    static async updateAirtableIdForNewlySyncedNotes({ records, engine, logger, }) {
        const out = await Promise.all(records.map(async (ent) => {
            ...
            const noteAirtableId = lodash_1.default.get(note.custom, "airtableId");
            if (!noteAirtableId) {
                const updatedNote = {
                    ...note,
