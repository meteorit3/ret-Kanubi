...
class GDocImportPod extends basev3_1.ImportPod {
    constructor() {
        ...
        this.createNote = async (opts) => {
            const { note, engine, wsRoot, vault, confirmOverwrite, onPrompt, importComments, } = opts;
            const existingNote = common_all_1.NoteUtils.getNoteByFnameV5({
                fname: note.fname,
                notes: engine.notes,
                vault,
                wsRoot,
    ...
    async plant(opts) {
        ...
        if (common_all_1.Time.now().toSeconds() > expirationTime) {
            accessToken = await utils_1.PodUtils.refreshGoogleAccessToken(wsRoot, refreshToken);
        }
