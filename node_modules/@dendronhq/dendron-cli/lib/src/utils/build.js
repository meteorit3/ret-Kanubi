...
class BuildUtils {
    ...
    static bump11ty(opts) {
        const root = this.getPluginRootPath();
        const sitePath = path_1.default.join(root, "src", "utils", "site.ts");
        const dst = `dendron-11ty": "^${opts.nextVersion.replace(/^0./, "1.")}"`;
        const newContent = fs_extra_1.default
            .readFileSync(sitePath, { encoding: "utf8" })
            .replace(/dendron-11ty.*/, dst);
        fs_extra_1.default.writeFileSync(sitePath, newContent);
    ...
    static async compilePlugin({ fast, quiet, }) {
     ...
     * @returns
     */
    static async packagePluginDependencies({ fast, quiet, }) {
