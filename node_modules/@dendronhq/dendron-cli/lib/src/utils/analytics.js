...
class CLIAnalyticsUtils {
    static track(event, props) {
        ...
        common_server_1.SegmentUtils.track(event, { type: "cli", cliVersion }, props);
    }
    static async trackSync(event, props) {
        const cliVersion = cli_1.CLIUtils.getClientVersion();
        await common_server_1.SegmentUtils.trackSync(event, { type: "cli", cliVersion }, props);
    }
    static identify() {
        const cliVersion = cli_1.CLIUtils.getClientVersion();
        common_server_1.SegmentUtils.identify({ type: "cli", cliVersion });
    }
    /**
