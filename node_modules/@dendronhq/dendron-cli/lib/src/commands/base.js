...
class CLICommand extends BaseCommand {
    constructor(opts) {
        ...
        this.eval = async (args) => {
            ...
            if ((0, common_all_1.isDendronResp)(out) && out.error) {
    ...
    async validateConfig(opts) {
        ...
        const configVersion = engine_server_1.DConfig.getRaw(wsRoot).version;
        ...
        if (!validationResp.isValid) {
            ...
            const instruction = reason === "client"
                ...
                : `Please make sure dendron.yml is up to date by running the following: \n dendron dev run_migration --migrationVersion=${engine_server_1.ALL_MIGRATIONS[0].version}`;
    ...
    addArgsToPayload(data) {
        lodash_1.default.set(this._analyticsPayload, "args", data);
    }
    print(obj) {
        if (!this.opts.quiet) {
            // eslint-disable-next-line no-console
