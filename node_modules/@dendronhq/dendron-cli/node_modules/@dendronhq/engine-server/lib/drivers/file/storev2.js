...
class FileStorage {
    ...
    async _initSchema(vault) {
        ...
        this.logger.info({ ctx, msg: "enter" });
        const vpath = (0, common_server_1.vault2Path)({ vault, wsRoot: this.wsRoot });
        const schemaFiles = (0, common_server_1.getAllFiles)({
            root: vpath,
            include: ["*.schema.yml"],
        });
    ...
    async initNotes() {
        ...
        const out = await Promise.all(this.vaults.map(async (vault) => {
            ...
            if (!this.engine.config.noCaching) {
                (0, utils_2.writeNotesToCache)(vpath, newCache);
            }
            return notes;
        }));
    ...
    async _initNotes(vault) {
        ...
        const noteFiles = (0, common_server_1.getAllFiles)({
            root: vpath,
            include: ["*.md"],
        });
        let errors = [];
        const cache = !this.engine.config.noCaching
            ? (0, utils_2.readNotesFromCache)(vpath)
            : { version: 0, notes: {} };
    ...
    async _writeNewNote({ note, existingNote, opts, }) {
        ...
        if (existingNote) {
            ...
            const parentNote = this.notes[existingNote.parent];
            // remove existing note from parent's children
            parentNote.children = lodash_1.default.reject(parentNote.children, (ent) => ent === existingNote.id);
            // update parent's children
            this.notes[existingNote.parent].children = parentNote.children;
