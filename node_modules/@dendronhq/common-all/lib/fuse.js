...
exports.NoteLookupUtils = exports.FuseEngine = exports.createSerializedFuseNoteIndex = exports.createFuseNote = exports.FuseExtendedSearchConstants = void 0;
...
class FuseEngine {
    ...
    static sortResults({ results, originalQS, }) {
        const groupedByScore = lodash_1.default.groupBy(results, (r) => r.score);
        ...
        const scores = lodash_1.default.keys(groupedByScore).map((stringKey) => ({
            key: stringKey,
        ...
        for (const score of scores) {
            ...
            const [stubs, notes] = lodash_1.default.partition(sameScoreMatches, (m) => m.item.stub);
            notes.sort((a, b) => {
...
class NoteLookupUtils {
    static async lookup({ qs, engine, showDirectChildrenOnly, }) {
        const { notes } = engine;
...
NoteLookupUtils.fetchRootResults = (notes) => {
    const roots = _1.NoteUtils.getRoots(notes);
