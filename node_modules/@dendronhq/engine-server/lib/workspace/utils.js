...
class WorkspaceUtils {
    ...
    static getWorkspaceTypeFromDir(dir) {
        if (fs_extra_1.default.pathExistsSync(path_1.default.join(dir, common_all_1.CONSTANTS.DENDRON_WS_NAME))) {
            return common_all_1.WorkspaceType.CODE;
        }
        if (fs_extra_1.default.pathExistsSync(path_1.default.join(dir, common_all_1.CONSTANTS.DENDRON_CONFIG_FILE))) {
            return common_all_1.WorkspaceType.NATIVE;
        }
        return common_all_1.WorkspaceType.NONE;
    }
    ...
    static findWSRoot() {
        ...
        const configPath = common_server_1.findUpTo({
            base: cwd,
            fname: "dendron.yml",
            maxLvl: 3,
            returnDirPath: true,
    ...
    static findWSRootInWorkspaceFolders(workspaceFolders) {
        const dendronWorkspaceFolders = workspaceFolders.filter((ent) => {
            return fs_extra_1.default.pathExistsSync(path_1.default.join(ent.uri.fsPath, "dendron.yml"));
        }) || [];
    ...
    static isPathInWorkspace({ wsRoot, vaults, fpath, }) {
        try {
            common_all_1.VaultUtils.getVaultByNotePath({
                vaults,
                wsRoot,
                fsPath: fpath,
            });
    ...
    static noteContentChanged({ content, note, }) {
        const noteHash = common_server_1.genHash(content);
    ...
    static getNoteUrl(opts) {
        ...
        if (anchor) {
            if (!common_all_1.isBlockAnchor(anchor)) {
                link += `#${common_all_1.getSlugger().slug(anchor)}`;
            }
            else {
                link += `#${anchor}`;
            }
