...
class DendronEngineV2 {
    ...
    static create({ wsRoot, logger }) {
        ...
        const config = lodash_1.default.defaultsDeep(common_server_1.readYAML(cpath), common_all_1.ConfigUtils.genDefaultConfig());
    ...
    async renderNote({ id }) {
        ...
        const beforeRenderMillis = common_all_1.milliseconds();
        ...
        const duration = common_all_1.milliseconds() - beforeRenderMillis;
    ...
    async _renderNote(note) {
        const proc = markdown_1.MDUtilsV5.procRehypeFull({
            engine: this,
            fname: note.fname,
            vault: note.vault,
            config: this.config,
        }, { flavor: markdown_1.ProcFlavor.PREVIEW });
        const payload = await proc.process(common_all_1.NoteUtils.serialize(note));
        const noteHtml = payload.toString();
        const renderedNote = common_all_1.ConfigUtils.getProp(this.config, "useNunjucks")
            ? markdown_1.nunjucks.renderString(noteHtml, {
    ...
    async refreshNotesV2(notes) {
        const notesMap = common_all_1.NoteUtils.createFnameNoteMap(lodash_1.default.values(this.notes), true);
    ...
    async renameNote(opts) {
        try {
            ...
            if (err instanceof common_all_1.DendronError)
                error = common_all_1.error2PlainObject(err);
    ...
    async addAccessTokensToPodConfig(opts) {
        ...
        let podConfig = common_server_1.readYAML(path);
        podConfig = {
            ...podConfig,
            accessToken,
            expirationTime,
        ...
        common_server_1.writeYAML(path, podConfig);
    }
