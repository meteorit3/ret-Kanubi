...
function handleImage({ proc, node, useFullPathUrl = false, }) {
    ...
    const logger = common_server_1.createLogger("handleImage");
    // ignore web images
    ...
    const fpath = path_1.default.join(common_server_1.vault2Path({ vault, wsRoot }), node.url);
    if (useFullPathUrl === true) {
        logger.info({ ctx, wsRoot, vault, url: node.url, fpath, useFullPathUrl });
        node.url = fpath;
        return;
    }
    const port = utils_1.EngineUtils.getEnginePort({ wsRoot });
...
function plugin(_opts) {
    ...
    function transformer(tree, _file) {
        unist_util_visit_1.default(tree, (node, _index, _parent) => {
