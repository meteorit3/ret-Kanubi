...
class ImageNodeHandler extends DendronNodeHander {
    ...
    static handle(node, { proc, cOpts }) {
        ...
        const assetsPrefix = utilsv5_1.MDUtilsV5.isV5Active(proc)
            ? config === null || config === void 0 ? void 0 : config.site.assetsPrefix
            : cOpts === null || cOpts === void 0 ? void 0 : cOpts.assetsPrefix;
        const imageNode = node;
        if (assetsPrefix) {
            imageNode.url =
                "/" + lodash_1.default.trim(assetsPrefix, "/") + "/" + lodash_1.default.trim(imageNode.url, "/");
        }
...
function plugin(opts) {
    ...
    function transformer(tree, _file) {
        ...
        if (mode !== utilsv5_1.ProcMode.IMPORT && !insideNoteRef && root.children) {
            ...
            const note = utilsv5_1.MDUtilsV5.getNoteByFname(proc, { fname });
            if (!note) {
        ...
        unist_util_visit_parents_1.default(tree, (node, ancestors) => {
            var _a;
            ...
            if (node.type === types_1.DendronASTTypes.USERTAG) {
                ...
                // but not if they are inside of a link, otherwise they break link rendering.
                if (!ancestors.some((node) => utils_2.RemarkUtils.isLink(node))) {
                    node = utils_2.userTag2WikiLinkNoteV4(userTag);
                }
                else {
                    node = mdast_builder_1.text(userTag.value);
                }
                parent.children[parentIndex] = node;
            }
            if (node.type === types_1.DendronASTTypes.WIKI_LINK &&
                ...
                if (engineError) {
                    utils_2.addError(proc, engineError);
                }
                let error;
                let note;
                if (mode !== utilsv5_1.ProcMode.IMPORT) {
                    ...
                    const out = utils_2.getNoteOrError(notes, value);
                    error = out.error;
                    note = out.note;
                }
                let color;
                if (mode !== utilsv5_1.ProcMode.IMPORT && value.startsWith(common_all_1.TAGS_HIERARCHY)) {
                    ...
                    if (colorType === "configured" ||
                        (!config.site.noRandomlyColoredTags && !(opts === null || opts === void 0 ? void 0 : opts.noRandomlyColoredTags))) {
                        color = maybeColor;
                    }
                }
                ...
                const maybeFileExtension = lodash_1.default.isBoolean(usePrettyLinks) && usePrettyLinks ? "" : ".html";
                // in v4, copts.prefix = absUrl + "/" + siteNotesDir + "/";
                // if v5, copts.prefix = ""
                let href;
                if (utilsv5_1.MDUtilsV5.isV5Active(proc)) {
                    href = `${(copts === null || copts === void 0 ? void 0 : copts.prefix) || ""}${value}${maybeFileExtension}${data.anchorHeader ? "#" + data.anchorHeader : ""}`;
                }
                else {
                    href = `${(copts === null || copts === void 0 ? void 0 : copts.prefix) || ""}${value}${maybeFileExtension}${data.anchorHeader ? "#" + data.anchorHeader : ""}`;
                }
            ...
            if (node.type === types_1.DendronASTTypes.BLOCK_ANCHOR) {
                ...
                const procOpts = utils_1.MDUtilsV4.getProcOpts(proc);
                const anchorHTML = blockAnchors_1.blockAnchor2html(node, procOpts.blockAnchorsOpts);
                let target;
                const grandParent = ancestors[ancestors.length - 2];
                if (utils_2.RemarkUtils.isParagraph(parent) &&
                    parent.children.length === 1 &&
                    common_all_1.isNotUndefined(grandParent) &&
                    ...
                    if (common_all_1.isNotUndefined(greatGrandParent) &&
                ...
                if (utils_2.RemarkUtils.isParent(target)) {
                    ...
                    const targetWrapper = mdast_builder_1.paragraph([
                        anchorHTML,
            ...
            if (node.type === types_1.DendronASTTypes.EXTENDED_IMAGE &&
                ...
                parent.children.splice(index, 1, extendedImage_1.extendedImage2html(node));
            }
