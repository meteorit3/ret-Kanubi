...
const getLinks = ({ ast, note, filter, }) => {
    const wikiLinks = [];
    const noteRefs = [];
    unist_util_visit_1.default(ast, (node) => {
        switch (node.type) {
    ...
    const dlinks = [];
    if (common_all_1.isNotUndefined(note.tags)) {
        let tags;
...
const getLinkCandidates = ({ ast, note, notesMap, }) => {
    const textNodes = [];
    unist_util_visit_1.default(ast, [types_1.DendronASTTypes.TEXT], (node, _index, parent) => {
        ...
        value.split(/\s+/).filter((word) => {
            ...
            if (maybeNote !== undefined) {
                const candidate = {
                    ...
                    to: {
                        fname: word,
                        vaultName: common_all_1.VaultUtils.getName(maybeNote.vault),
                    },
                };
...
class LinkUtils {
    ...
    static findLinkCandidates({ note, 
    // notes,
    notesMap, engine, }) {
        const content = note.body;
...
class AnchorUtils {
    ...
    static headerText(header) {
        const headerText = [];
        unist_util_visit_1.default(header, (node) => {
            switch (node.type) {
    ...
    static headerTextPosition(header) {
        let start;
        let end;
        unist_util_visit_1.default(header, [
    ...
    static anchorNode2anchor(node, slugger) {
        ...
        if (node.type === types_1.DendronASTTypes.HEADING) {
            const text = this.headerText(node);
            const value = slugger.slug(this.headerText(node));
            return [
                value,
                {
                    type: "header",
            ...
            common_all_1.assertUnreachable(node);
        }
    }
    static async findAnchors(opts) {
        ...
        try {
            ...
            const error = common_all_1.DendronError.createFromStatus({
                ...
                payload: { note: common_all_1.NoteUtils.toLogObj(opts.note), wsRoot: opts.wsRoot },
                innerError: err,
            });
    ...
    static anchor2string(anchor) {
        ...
        if (anchor.type === "header")
            return anchor.value;
        common_all_1.assertUnreachable(anchor.type);
    }
}
...
class RemarkUtils {
    ...
    static convertLinksToDotNotation(note, changes) {
        return function () {
            return (tree, _vfile) => {
                const root = tree;
                const wikiLinks = unist_util_select_1.selectAll(types_1.DendronASTTypes.WIKI_LINK, root);
    ...
    static convertAssetReferences(note, assetHashMap, changes) {
        return function () {
            return (tree, _vfile) => {
                const root = tree;
                const assetReferences = [
                    ...unist_util_select_1.selectAll(types_1.DendronASTTypes.IMAGE, root),
                    ...unist_util_select_1.selectAll(types_1.DendronASTTypes.LINK, root),
                ];
                assetReferences.forEach((asset) => {
                    const key = lodash_1.default.replace(asset.url, /[\\|/|.]/g, "");
    ...
    static convertWikiLinkToNoteUrl(note, changes, engine, dendronConfig) {
        return function () {
            return (tree, _vfile) => {
                ...
                const links = LinkUtils.findLinks({ note, engine }).filter((linkNode) => linkNode.type === "wiki");
                let dirty = false;
                links.forEach((linkNode, i) => {
                    var _a, _b, _c, _d;
                    let vault;
                    // If the link specifies a vault, we should only look at that vault
                    if (linkNode.to && !lodash_1.default.isUndefined((_a = linkNode.to) === null || _a === void 0 ? void 0 : _a.vaultName)) {
                        vault = common_all_1.VaultUtils.getVaultByName({
                            vaults: engine.vaults,
                            vname: (_b = linkNode.to) === null || _b === void 0 ? void 0 : _b.vaultName,
                        });
                    }
                    ...
                    if (existingNote) {
                        const urlRoot = ((_c = dendronConfig.site) === null || _c === void 0 ? void 0 : _c.siteUrl) || "";
                        const { vault } = existingNote;
                        wikiLinks[i]["value"] = workspace_1.WorkspaceUtils.getNoteUrl({
                            config: dendronConfig,
                            note: existingNote,
                            vault,
                            urlRoot,
                            anchor: (_d = linkNode.to) === null || _d === void 0 ? void 0 : _d.anchorHeader,
                        });
                        dirty = true;
                    }
                });
    ...
    static oldNoteRef2NewNoteRef(note, changes) {
        return function () {
            return (tree, _vfile) => {
                const root = tree;
                //@ts-ignore
                const notesRefLegacy = unist_util_select_1.selectAll(types_1.DendronASTTypes.REF_LINK, root);
    ...
    static extractHeaderBlock(tree, targetHeader) {
        ...
        let nextHeaderIndex;
        unist_util_visit_1.default(tree, (node, index) => {
            if (nextHeaderIndex) {
                return;
            }
    ...
    static extractFootnoteDefs(root) {
        return unist_util_select_1.selectAll(types_1.DendronASTTypes.FOOTNOTE_DEFINITION, root).filter(RemarkUtils.isFootnoteDefinition);
    }
}
    ...
    if (lodash_1.default.isEqual(Object.keys(a).sort(), Object.keys(b).sort())) {
        ...
        if (lodash_1.default.isEqual(aOmit, bOmit)) {
            if (lodash_1.default.has(a, "children")) {
                return lodash_1.default.every(a.children, (aChild, aIndex) => {
