...
class TwitterCard extends BaseTag {
  async liquidRender(scope, hash) {
    ...
    const imageFromContext = context ? context["note"]?.custom?.image : undefined;
    // Fallback on using image in config if available and none is set in front matter.
    const hasConfigImageOnly = !imageFromContext && this.config.image;
    let excerpt = undefined;
    if (context["note"]?.custom?.excerpt) {
      excerpt = context["note"]?.custom?.excerpt;
    }
    ...
    const baseImage = imageFromContext && imageFromContext.startsWith("https://") ? imageFromContext : this.config.url + image;
